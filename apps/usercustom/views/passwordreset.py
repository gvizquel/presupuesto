        ...
        if "uidb64" in self.kwargs and "token" in self.kwargs:
            ...
            if user is not None and token_valid:
                ...
                messages.success(
                    self.request,
                    _(
                        "Welcome back, your password has been successfully modified. Please log in using your credentials."
                    ),
                )
            else:
                messages.error(
            ...
            if user is not None:
                ...
                messages.success(
                    self.request,
                    _(
                        "Instructions to recover your password in %(proj_name)s were sent to the email account %(email)s"
                    )
                    % {"proj_name": settings.PROJECT_NAME, "email": email},
                )
