    ...
    def get(self, request, *args, **kwargs):
        ...
        if user is not None and token_valid:
            ...
            messages.success(
                self.request,
                _(
                    "Welcome, your account has been successfully activated. Please log in using your credentials."
                ),
            )
        else:
            messages.error(
                self.request,
                _(
                    "The sign up confirm link is invalid. If your account is not yet active, use the password recovery link."
                ),
            )
